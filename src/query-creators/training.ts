...
export const addTrainingToProgram = () => {
    return `
        INSERT INTO training_program_link (
...
export const deleteTrainingFromProgram = () => 'DELETE FROM training_program_link WHERE id = $1 RETURNING *';
...
export const deleteTraining = () => 'DELETE FROM training WHERE id = $1 RETURNING *';
export const getTrainingById = () => 'SELECT * FROM training WHERE id = $1';

export const getDailyTrainings = (order: OrderType = OrderType.ASC) => {
    return `SELECT * FROM training WHERE is_daily = true ORDER BY created_date ${order} LIMIT $1 OFFSET $2`;
};

export const getTrainings = (order: OrderType = OrderType.ASC, fields: string[]) => {
    return `
...
export const deleteTrainingRound = () => 'DELETE FROM training_round WHERE id = $1 RETURNING *';
export const updateTrainingRound = (fields: string[]) => updateQuery(fields, 'training_round');
export const getTrainingRoundsByTraining = () => 'SELECT * FROM training_round WHERE training_id = $1';

// TODO create ignores in db if is_hidden = true
export const saveTraining = () => {
    return `
        INSERT INTO user_training_saved (
            user_id, training_id
        ) VALUES (
            $1, $2
        ) RETURNING *
...
export const getUserSavedTrainings = (order: OrderType = OrderType.ASC) => {
    return `
        SELECT * FROM training
