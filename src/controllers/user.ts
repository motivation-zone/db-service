...
import {stringToBoolean, checkGetLimitParameters} from '../utils';
import {API_URLS} from '../urls';

const userController = express();
const urls = API_URLS.user;

userController.post(urls.create, async (req: Request, res: Response) => {
...
userController.post(urls.updateById, async (req: Request, res: Response) => {
    const {id} = req.params;
    const user = new UserModel(req.body, {clearNotUpdatedFields: true});
    user.id = id;
